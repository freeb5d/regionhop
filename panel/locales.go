...
var translations = map[string]map[string]string{
	"en": {
		...
		"backup.title":          "Backup",
		"backup.desc":           "Export everything needed to recreate your locations on a freshly-installed server, or import a backup file.",
		"backup.export":         "Export backup",
		"backup.import":         "Import backup",
		"backup.note":           "The backup is a single JSON file containing your locations (names, regions, ports) and your Psiphon config. Importing only adds locations that don't already exist — it never overwrites or removes anything.",
		"backup.import.added":   "Added %d location(s).",
		"backup.import.skipped": "Skipped %d:",
		"upstream.title":        "Upstream",
		"upstream.desc":         "Route every location's connection to Psiphon through another proxy first — for servers whose own network blocks or throttles Psiphon.",
		"upstream.saved":        "Saved. Running locations were restarted to use it.",
		"upstream.mode":         "Mode",
		"upstream.mode.none":    "Direct (no upstream)",
		"upstream.mode.proxy":   "SOCKS / HTTP proxy",
		"upstream.mode.v2ray":   "V2Ray (built-in Xray)",
		"upstream.noxray":       "not available on this server",
		"upstream.proxy.label":  "Proxy URL",
		"upstream.v2ray.label":  "Share link or Xray outbound JSON",
		"upstream.service":      "Xray service",
		"upstream.submit":       "Save upstream",
		"upstream.note":         "Applies to all locations immediately. Proxy URL: socks5://, socks4a:// or http://, optionally with user:pass@. V2Ray: paste a vless://, vmess://, trojan:// or ss:// link, or an Xray outbound JSON; regionhop runs it locally on 127.0.0.1 only.",
		"logs.title":            "Logs: %s",
	...
	"fa": {
		...
		"backup.desc":           "هرچه برای بازسازی لوکیشن‌ها روی یک سرور تازه‌نصب‌شده لازم است خروجی بگیرید، یا یک فایل پشتیبان وارد کنید.",
		"backup.export":         "خروجی گرفتن از پشتیبان",
		"backup.import":         "وارد کردن پشتیبان",
		"backup.note":           "فایل پشتیبان یک فایل JSON تکی است که شامل لوکیشن‌های شما (نام، منطقه، پورت) و کانفیگ Psiphon شماست. وارد کردن فقط لوکیشن‌هایی را اضافه می‌کند که از قبل وجود ندارند — هرگز چیزی را بازنویسی یا حذف نمی‌کند.",
		"backup.import.added":   "%d لوکیشن اضافه شد.",
		"backup.import.skipped": "%d مورد رد شد:",
		"upstream.title":        "آپستریم",
		"upstream.desc":         "اتصال همه لوکیشن‌ها به سرورهای Psiphon را اول از یک پراکسی دیگر عبور دهید — برای سرورهایی که شبکه‌شان Psiphon را مسدود یا محدود می‌کند.",
		"upstream.saved":        "ذخیره شد. لوکیشن‌های فعال برای استفاده از آن ری‌استارت شدند.",
		"upstream.mode":         "حالت",
		"upstream.mode.none":    "مستقیم (بدون آپستریم)",
		"upstream.mode.proxy":   "پراکسی SOCKS / HTTP",
		"upstream.mode.v2ray":   "V2Ray (Xray داخلی)",
		"upstream.noxray":       "روی این سرور در دسترس نیست",
		"upstream.proxy.label":  "آدرس پراکسی",
		"upstream.v2ray.label":  "لینک اشتراک یا JSON خروجی Xray",
		"upstream.service":      "سرویس Xray",
		"upstream.submit":       "ذخیره آپستریم",
		"upstream.note":         "بلافاصله روی همه لوکیشن‌ها اعمال می‌شود. آدرس پراکسی: socks5:// یا socks4a:// یا http://، در صورت نیاز همراه با user:pass@. برای V2Ray: یک لینک vless:// ،vmess:// ،trojan:// یا ss:// یا JSON خروجی Xray وارد کنید؛ regionhop آن را فقط روی 127.0.0.1 اجرا می‌کند.",
		"logs.title":            "لاگ‌ها: %s",
	...
	"ar": {
		...
		"backup.desc":           "صدّر كل ما يلزم لإعادة إنشاء مواقعك على خادم مُثبَّت حديثًا، أو استورد ملف نسخة احتياطية.",
		"backup.export":         "تصدير نسخة احتياطية",
		"backup.import":         "استيراد نسخة احتياطية",
		"backup.note":           "النسخة الاحتياطية ملف JSON واحد يحتوي على مواقعك (الأسماء والمناطق والمنافذ) وتكوين Psiphon الخاص بك. الاستيراد يضيف فقط المواقع غير الموجودة مسبقًا — ولا يقوم أبدًا بالكتابة فوق أي شيء أو حذفه.",
		"backup.import.added":   "تمت إضافة %d موقع.",
		"backup.import.skipped": "تم تخطي %d:",
		"upstream.title":        "الوكيل الأمامي",
		"upstream.desc":         "مرّر اتصال كل المواقع بخوادم Psiphon عبر وكيل آخر أولًا — للخوادم التي تحظر شبكتها Psiphon أو تبطئه.",
		"upstream.saved":        "تم الحفظ. أُعيد تشغيل المواقع العاملة لاستخدامه.",
		"upstream.mode":         "الوضع",
		"upstream.mode.none":    "مباشر (بدون وكيل أمامي)",
		"upstream.mode.proxy":   "وكيل SOCKS / HTTP",
		"upstream.mode.v2ray":   "V2Ray (Xray مدمج)",
		"upstream.noxray":       "غير متوفر على هذا الخادم",
		"upstream.proxy.label":  "عنوان الوكيل",
		"upstream.v2ray.label":  "رابط المشاركة أو JSON لمخرج Xray",
		"upstream.service":      "خدمة Xray",
		"upstream.submit":       "حفظ الوكيل الأمامي",
		"upstream.note":         "يُطبَّق على كل المواقع فورًا. عنوان الوكيل: socks5:// أو socks4a:// أو http://، مع user:pass@ اختياريًا. لـ V2Ray: الصق رابط vless:// أو vmess:// أو trojan:// أو ss:// أو JSON لمخرج Xray؛ يشغّله regionhop محليًا على 127.0.0.1 فقط.",
	...
	"ru": {
		...
		"backup.desc":           "Экспортируйте всё необходимое для восстановления локаций на свежеустановленном сервере, или импортируйте файл резервной копии.",
		"backup.export":         "Экспортировать резервную копию",
		"backup.import":         "Импортировать резервную копию",
		"backup.note":           "Резервная копия — это один JSON-файл с вашими локациями (имена, регионы, порты) и конфигурацией Psiphon. Импорт добавляет только те локации, которых ещё нет — он никогда ничего не перезаписывает и не удаляет.",
		"backup.import.added":   "Добавлено локаций: %d.",
		"backup.import.skipped": "Пропущено %d:",
		"upstream.title":        "Вышестоящий прокси",
		"upstream.desc":         "Пускать подключение всех локаций к серверам Psiphon сначала через другой прокси — для серверов, чья сеть блокирует или замедляет Psiphon.",
		"upstream.saved":        "Сохранено. Работающие локации перезапущены.",
		"upstream.mode":         "Режим",
		"upstream.mode.none":    "Напрямую (без прокси)",
		"upstream.mode.proxy":   "SOCKS / HTTP прокси",
		"upstream.mode.v2ray":   "V2Ray (встроенный Xray)",
		"upstream.noxray":       "недоступно на этом сервере",
		"upstream.proxy.label":  "URL прокси",
		"upstream.v2ray.label":  "Ссылка или JSON outbound для Xray",
		"upstream.service":      "Служба Xray",
		"upstream.submit":       "Сохранить",
		"upstream.note":         "Применяется ко всем локациям сразу. URL прокси: socks5://, socks4a:// или http://, при необходимости с user:pass@. V2Ray: вставьте ссылку vless://, vmess://, trojan:// или ss://, либо JSON outbound для Xray; regionhop запускает его только на 127.0.0.1.",
		"logs.title":            "Логи: %s",
	...
	"zh": {
		...
		"backup.title":          "备份",
		"backup.desc":           "导出在全新安装的服务器上重建所有位置所需的一切，或导入备份文件。",
		"backup.export":         "导出备份",
		"backup.import":         "导入备份",
		"backup.note":           "备份是一个包含你的位置（名称、地区、端口）和 Psiphon 配置的单个 JSON 文件。导入只会添加尚不存在的位置——绝不会覆盖或删除任何内容。",
		"backup.import.added":   "已添加 %d 个位置。",
		"backup.import.skipped": "已跳过 %d 个：",
		"upstream.title":        "上游代理",
		"upstream.desc":         "让所有位置连接 Psiphon 服务器时先经过另一个代理——适用于本机网络屏蔽或限速 Psiphon 的服务器。",
		"upstream.saved":        "已保存。运行中的位置已重启以使用它。",
		"upstream.mode":         "模式",
		"upstream.mode.none":    "直连（无上游）",
		"upstream.mode.proxy":   "SOCKS / HTTP 代理",
		"upstream.mode.v2ray":   "V2Ray（内置 Xray）",
		"upstream.noxray":       "此服务器不可用",
		"upstream.proxy.label":  "代理地址",
		"upstream.v2ray.label":  "分享链接或 Xray 出站 JSON",
		"upstream.service":      "Xray 服务",
		"upstream.submit":       "保存上游",
		"upstream.note":         "立即应用于所有位置。代理地址：socks5://、socks4a:// 或 http://，可带 user:pass@。V2Ray：粘贴 vless://、vmess://、trojan:// 或 ss:// 链接，或 Xray 出站 JSON；regionhop 仅在 127.0.0.1 上本地运行它。",
		"logs.title":            "日志：%s",
