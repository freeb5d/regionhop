...
var translations = map[string]map[string]string{
	"en": {
		...
		"health.ok":             "traffic ok",
		"health.fail":           "probe failed",
		"health.unknown":        "no probe yet",
		"health.tooltip":        "Independent probe through this location's own proxy, run every 5 minutes — confirms the tunnel is actually passing traffic, not just that it's connected.",
	...
	"fa": {
		...
		"health.ok":             "ترافیک برقرار است",
		"health.fail":           "بررسی ناموفق بود",
		"health.unknown":        "هنوز بررسی نشده",
		"health.tooltip":        "بررسی مستقل از طریق پراکسی خود این لوکیشن، هر ۵ دقیقه یک‌بار — تأیید می‌کند که تونل واقعاً ترافیک را رد و بدل می‌کند، نه فقط اینکه متصل است.",
	...
	"ar": {
		...
		"table.status":          "الحالة",
		"table.flag":            "العلم",
		"table.traffic":         "حركة البيانات",
		"table.actions":         "إجراءات",
		"table.empty":           "لا توجد مواقع بعد — أضف واحدًا أعلاه للبدء.",
		"health.ok":             "حركة البيانات تعمل",
		"health.fail":           "فشل الفحص",
		"health.unknown":        "لم يتم الفحص بعد",
		"health.tooltip":        "فحص مستقل عبر وكيل هذا الموقع نفسه، يُجرى كل 5 دقائق — يؤكد أن النفق ينقل بيانات فعليًا، وليس فقط أنه متصل.",
	...
	"ru": {
		...
		"health.ok":             "трафик идёт",
		"health.fail":           "проверка не пройдена",
		"health.unknown":        "проверок ещё не было",
		"health.tooltip":        "Независимая проверка через собственный прокси этой локации, каждые 5 минут — подтверждает, что туннель действительно передаёт трафик, а не просто подключён.",
	...
	"zh": {
		...
		"health.fail":           "探测失败",
		"health.unknown":        "尚未探测",
		"health.tooltip":        "通过该线路自身代理进行的独立探测，每 5 分钟一次——用于确认隧道确实在传输流量，而不仅仅是已连接。",
		"action.restart":        "重启",
		"action.stop":           "停止",
		"action.remove":         "删除",
