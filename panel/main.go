...
func main() {
	...
	propagationChannelID := envOr("PSIPHON_PROPAGATION_CHANNEL_ID", "")
	sponsorID := envOr("PSIPHON_SPONSOR_ID", "")
...
func (a *app) handleAdd(w http.ResponseWriter, r *http.Request) {
	...
	if err := writeTunnelConfig(configsDir, dataDir, t, a.propagationChannelID, a.sponsorID); err != nil {
...
func (a *app) handleSettings(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		pcid := strings.TrimSpace(r.FormValue("propagation_channel_id"))
		sid := strings.TrimSpace(r.FormValue("sponsor_id"))
		if err := persistEnvVar(panelEnvPath, "PSIPHON_PROPAGATION_CHANNEL_ID", pcid); err != nil {
			http.Error(w, "failed saving: "+err.Error(), 500)
			return
		}
		if err := persistEnvVar(panelEnvPath, "PSIPHON_SPONSOR_ID", sid); err != nil {
			http.Error(w, "failed saving: "+err.Error(), 500)
			return
		}
		a.propagationChannelID = pcid
		a.sponsorID = sid
	...
	tmpl.ExecuteTemplate(w, "settings.html", map[string]any{
		"PropagationChannelID": a.propagationChannelID,
